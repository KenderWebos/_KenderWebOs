...
int main(int argc, char *argv[])
{
  ...
  datos_compartidos.capacidad_almacen = 10;
  datos_compartidos.rondas = 1;

  datos_compartidos.demora_min_productor = 5;
  datos_compartidos.demora_max_productor = 15;

  datos_compartidos.demora_min_consumidor = 5;
  datos_compartidos.demora_max_consumidor = 15;
  ...
  int cantidad_hebras = random_entre(1, 10);
  ...
  for (int i = 0; i < cantidad_hebras; i++)
  {
    ...
    int result = pthread_create(&threads[i], NULL, produce, NULL);

    sleep(1);

    printw("Creando hilo %i\n", i);
  ...
  //   if (error != 0)
  //   {
  //     fprintw(stderr, "error: no puede crear consumidor\n");
  //     error = 1;
  //   }
  // }
  // else
  // {
  //   fprintw(stderr, "error: no puede crear productor\n");
  //   error = 1;
  // }
  ...
  printw("Tiempo de ejecución: %.9lfs\n", periodo);
...
void *consume(void *data)
{
  ...
  for (int ronda = 0; ronda < datos_compartidos->rondas; ++ronda)
  {
    ...
    for (int indice = 0; indice < datos_compartidos->capacidad_almacen; ++indice)
    {
      ...
      printw("\t\tIndice almacen %i se consume %lg\n", indice, value);
